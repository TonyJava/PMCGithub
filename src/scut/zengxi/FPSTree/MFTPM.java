...
/*		List<List<String>> transRecords = fstree
	 ...
	 * @return
	 */
    public static List<List<Map<String,String>>> getMFSResults(){
		List<List<Map<String,String>>> results = new LinkedList<>();
		init();
		try{
		...
		for (int i = 0; i < mftss.size(); i++) {
			List<Map<String,String>> mfs= new LinkedList<>();
			if(mftss.get(i).size()>0){
				Map<String,String> sup=new HashMap<>();
				sup.put("support",mftss.get(i).get(0).substring(4,mftss.get(i).get(0).length()-1));
				String sequence = "";
